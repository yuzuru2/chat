/**
 * エラーログ
 */
import { write_error_log } from 'src/util';

/**
 * db
 */
import { Mongodb, t_argument } from 'src/mongodb';
import { l_create_room } from 'src/logic/l_create_room';

/**
 * logic
 */
import { l_after_second } from 'src/logic/l_after_second';

/**
 * 定数
 */
import { constant } from 'src/constant';

export const a_create_room = async (params: {
  socket: SocketIO.Socket;
  jwt_token: string;
  ip: string;
  io: SocketIO.Server;
}) => {
  const _this_url = constant.URL_CREATE_ROOM;
  const _correct_url = constant.URL_LOUNGE;

  params.socket.on(_this_url, async (req: { upper: number; name: string }) => {
    try {
      // バリデーション
      if (typeof req.name !== 'string') {
        throw new Error('a_create_room');
      }

      if (typeof req.upper !== 'number') {
        throw new Error('a_create_room');
      }

      const _ret = await l_after_second({
        jwt_token: params.jwt_token,
        ip: params.ip
      });

      if (_ret.url !== _correct_url) {
        params.socket.emit(_this_url, _ret);
        return;
      }

      // 部屋作成
      await new Mongodb<string>().normalMethod(
        l_create_room({
          hostId: _ret.userId,
          name: req.name,
          upper: req.upper
        })
      );

      // レスポンス
      params.socket.emit(_this_url, { url: constant.URL_ROOM });
    } catch (e) {
      params.socket.disconnect();
      await write_error_log(e);
    }
  });
};
