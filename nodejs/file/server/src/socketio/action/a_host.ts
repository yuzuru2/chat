...
export const a_host = async (params: {
  ...
  params.socket.on(_this_url, async (req: { hostId: string }) => {
    try {
      // バリデーション
      if (typeof req.hostId !== 'string') {
        throw new Error('a_host');
      }
